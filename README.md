...
Poor Man's VFX. Use non-functional orbs to visually display stacks of a power (or some other number).
...
2. Implement IDisplayOrbGenerator\<T> on any class
...
4. You can set the maximumn number of orb slots for a character (affects DisplayOrbs only, min 10, and must be performed each comabat)
